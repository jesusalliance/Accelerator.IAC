...
# Place this AFTER all module calls (uat, uat, prod, shared) so outputs are available
...
resource "azurerm_virtual_network_peering" "hub_to_uat" {
  ...
  depends_on = [module.uat]                      # ← ADD this (optional but recommended)
}
...
resource "azurerm_virtual_network_peering" "uat_to_hub" {
  ...
  depends_on = [module.uat, module.shared]       # ← ADD
}
